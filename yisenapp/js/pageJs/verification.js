var newTokenId = null;
//校验信息的id
var verificationId = GetQueryString("id");
//  0 不可改 1 可改
...
function getTitleInfo(tokenId) {
	...
	var ajax = $.ajax({
		type: "post",
		url: url + "/handler/Verification.asmx/GetTitleInfo?jsoncallback?",
		dataType: "jsonp",
		jsonp: 'jsoncallback',
		data: {
			"tokenId": tokenId,
		...
		success: function(data) {
			...
			if(title.length != 1) {
				layer.alert("加载校验信息错误");
				return;
			}
			$("#jianxiuyuan").html(title[0].verification_emp_name);
			$("#shedingshijian").html(title[0].verification_datetime_set);
			$("#xianchangmingcheng").html(title[0].scene_name);
		},
		error: function(data) {
			layer.close(loading);
			layer.msg("加载信息失败");
...
function getPositionName(tokenId) {
	...
	var ajax = $.ajax({
		type: "post",
		url: url + "/handler/Verification.asmx/GetPositionName?jsoncallback?",
		dataType: "jsonp",
		jsonp: 'jsoncallback',
		data: {
			"tokenId": tokenId,
...
function loadTableInfo(json, tokenId) {
	...
	var ajax = $.ajax({
		type: "post",
		url: url + "/handler/Verification.asmx/GetOtherinfo?jsoncallback?",
		dataType: "jsonp",
		jsonp: 'jsoncallback',
		data: {
			"verificationId": verificationId,
		...
		success: function(data) {
			...
			for(var a = 0; a < length; a++) {
				...
				for(var b = 0; b < map[json[a].item_position]; b++) {
					//下脚标
					var i = index + b;
					if(result[i].verification_status == "通过") {
						kgOnInf = "kg on";
					} else {
						kgOnInf = "kg off";
					}
					...
					if(infoFlag) {
						...
						str += '<tr>' +
							'<td style="display:none">' + result[i].verification_detail_id + '</td>' +
							'<td style="display:none">' + result[i].verification_memo + '</td>' +
							...
							//											'<div class="window">'+
							//												'<p>备注：</p>'+
							//												'<textarea name="" rows="" cols="">'+result[i].verification_memo+'</textarea>'+
							//												'<div>'+
							//													'<button onclick="modifyMemo(this)">修改</button>'+
...
//	console.log(status,detailid)
	var loading = layer.load(2, {
		shade: [0.2, '#000']
	});
	var ajax = $.ajax({
		type: "post",
		url: url + "/handler/Verification.asmx/SaveVerificationInfo?jsoncallback?",
		dataType: "jsonp",
		jsonp: 'jsoncallback',
		data: {
			"tokenId": newTokenId,
...
function saveMemoInfo(content, detailid) {
	...
	$.ajax({
		type: "post",
		url: url + "/handler/Verification.asmx/saveMemoInfo?jsoncallback?",
		dataType: "jsonp",
		jsonp: 'jsoncallback',
		data: {
			"tokenId": newTokenId,
...
//提交--需要verificationid
function sumbitInfo(verificationid) {
...
function sendAjax() {
	...
	var ajax = $.ajax({
		type: "post",
		url: url + "/handler/Verification.asmx/SumbitVerificationInfo?jsoncallback?",
		dataType: "jsonp",
		jsonp: 'jsoncallback',
		data: {
			"tokenId": newTokenId,
		...
		success: function(data) {
			...
			layer.msg("保存成功");
			setTimeout(function() {
				window.location.href = "verification_info.html";
			}, 1314)
		},
		error: function(data) {
			$(".btnBox").show();
